...
n_clusters = 10  # Set this to 8 based on your clustering needs
...
selected_proteins = ['DAPI', 'aSMA', 'CD11c', 'Vimentin', 'CD45', 'CD3', 'CD31', 'p21', 'CD4', 'CD19']
# Example: If these proteins correspond to specific channel indices in your data, you can map them here
channel_indices = [0, 2, 6, 7, 8, 9, 11, 15, 18, 20]  # Replace with correct indices for selected proteins
...
for core_filename in os.listdir(core_dir):
    if core_filename.startswith('reg00') and not (core_filename.startswith('reg007') or core_filename.startswith('reg0010')) and (core_filename.endswith('.tif') or core_filename.endswith('.qptiff')):
        ...
        with tifffile.TiffFile(file_path) as tif:
            ...
            dapi_signal_threshold = 5  # Adjust this threshold based on your data
...
def plot_clustered_core(core_filename):
    ...
    # Plot the clustered core
    plt.imshow(clustered_image, cmap=custom_cmap, vmin=-1, vmax=n_clusters-1)  # Plot with custom colormap
...
plot_clustered_core('reg002_X01_Y01_Z01.tif')
