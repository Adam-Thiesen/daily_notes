...
class SimpleTransformerWithClassToken(nn.Module):
    def __init__(
        ...
        # Embedding to go from input_dim (e.g. 768) to embed_dim
        self.embedding = nn.Linear(input_dim, embed_dim)

        # A trainable [CLS] token parameter
        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim))

        # We will instantiate PositionalEncoding dynamically in forward
        ...
        self.transformer_layer = nn.TransformerEncoderLayer(
            ...
            dropout=dropout,
            batch_first=True  # so the shape is (batch, seq, embed_dim)
        )
        ...
        # We will store attention weights if needed for analysis
    ...
    def forward(self, x):
        ...
        # Create or update the positional encoding if needed
        if (self.positional_encoding is None) or (x.size(1) + 1 > self.positional_encoding.pe.size(0)):
            # +1 for the class token
            self.positional_encoding = PositionalEncoding(
                embed_dim=self.embedding.out_features,
                max_len=x.size(1) + 1  # we need enough positions for CLS + tiles
            )

        # Expand [CLS] token to the current batch size
        # Shape after expand: (batch_size, 1, embed_dim)
        cls_tokens = self.cls_token.expand(batch_size, -1, -1)

        # Concatenate [CLS] token at the beginning of the sequence
        # New shape: (batch_size, 1 + num_tiles, embed_dim)
        x = torch.cat((cls_tokens, x), dim=1)

        # Apply positional encoding
        # Output shape remains (batch_size, seq_len, embed_dim)
        x = self.positional_encoding(x)

        x = self.dropout(x)

        # Pass through the Transformer encoder
        # Since we set batch_first=True above, x remains (batch_size, seq_len, embed_dim)
        x = self.transformer(x)

        # The first token (index 0) in the sequence is our [CLS] token
        # We'll use that for classification
        cls_output = x[:, 0]  # shape: (batch_size, embed_dim)

        # Optional: store attention weights if you want to analyze them
        # (requires a custom implementation or hooking into the Transformer forward pass)
        # self.attention_weights = ...

        # Pass [CLS] output through a dropout and then the classification layer
        cls_output = self.dropout(cls_output)
        logits = self.fc(cls_output)

        # Return shape: (batch_size,) for binary classification
        return logits.squeeze(1), None  # We return None for attn_weights here
