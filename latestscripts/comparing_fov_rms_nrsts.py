...
def process_combination(file1, file2):
    ...
    #print(f"   merged_df shape before alveolar embryonal filter = {merged_df.shape}")
    filtered_df = merged_df[
        merged_df['Histological Type'].isin(['NRTS', 'RMS'])
    ]
    #print(f"   merged_df shape before Tissue ID filter = {filtered_df.shape}")
...
def plot_confusion_matrix(cm, label1, label2, output_dir):
    ...
    plt.savefig(os.path.join(output_dir, f"nrsts_rms_new_confusion_matrix_{label1}_vs_{label2}.pdf"), dpi=300, transparent=True)
    plt.close()
...
def plot_single_aucs_heatmap(
    ...
    os.makedirs(output_dir, exist_ok=True)
    plt.savefig(os.path.join(output_dir, "nrsts_rms_new2_heatmap_single_files.pdf"), dpi=300, transparent=True)
    plt.close()
...
def plot_triangle_heatmap_with_light_grey_diag(
    ...
    os.makedirs(output_dir, exist_ok=True)
    plt.savefig(os.path.join(output_dir, f"nrsts_rms_triangle_heatmap_lightgrey_diag_{backbone}.pdf"), dpi=300, transparent=True)
    plt.close()
...
def plot_boxplot_and_compute_pvals(
    ...
    # Save figure
    os.makedirs(output_dir, exist_ok=True)
    plt.savefig(os.path.join(output_dir, f"nrsts_rms_boxplot_single_{backbone}.pdf"), dpi=300, transparent=True)
    plt.close()
...
def compare_backbones_for_fov(
    ...
    # Save figure
    os.makedirs(output_dir, exist_ok=True)
    plt.savefig(os.path.join(output_dir, f"nrsts_rms_barplot_backbones_FOV{fov}.pdf"), dpi=300, transparent=True)
    plt.close()
    ...
    for backbone in BACKBONES:
        ...
        for (label1, label2, cm), (_, _, metrics) in zip(confusion_matrices, metrics_list):
            if cm is not None:
                ...
                print("Plotting Confusion Matrix not")
